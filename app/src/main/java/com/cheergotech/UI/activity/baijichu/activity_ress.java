 ...
 * 发贴子
 */
public class activity_ress extends BaseActivity {
    @BindView(R.id.recyclerview)
    RecyclerView recyclerview;
    @BindView(R.id.easeInOut)
    EditText easeInOut;
    String Describes;
    ...
    public static void setAction(Context context) {
        ...
        intent.setClass(context, activity_ress.class);
    ...
    public void OnClick(View v) {
        switch (v.getId()) {
            case R.id.send1:
                Describes = easeInOut.getText().toString().trim();
                if (TextUtils.isEmpty(Describes)) {
                    Toasts.toastMessage(getString(R.string.taa));
                    return;
                }
                updadteImages();
                break;
     ...
     * @return
     */
    private String getDetailsImg() {
        StringBuffer stringBuffer = new StringBuffer();
        for (int i = 0; i < list.size(); i++) {
            String url = (String) list.get(i);
            if (i < list.size() - 1) {
                stringBuffer.append(String.format("%s,", url.trim()));
            } else {
                stringBuffer.append(String.format("%s", url.trim()));
            }
    ...
    private void sendclassTask(String path) {
        Datamodule.getInstance().classcirclesubmit(Msgconfig.getInstance().getClassId(), Describes, path, new CallBcak() {
            @Override
            public void Onfall() {
            ...
            public void OnSuccess() {
                dismissDialog();
                Toasts.toastMessage(getString(R.string.messagemsg));
