...
public class MediaPlayHelper {
    ...
    private static int sindex = 20; //音量
     ...
     * @return
     */
    public static MediaPlayHelper playHelper() {
        if (mediaPlayHelper == null) {
            mediaPlayHelper = new MediaPlayHelper(DemoApplication.instance());
        }
        return mediaPlayHelper;
    }
    ...
    private void start(String resPath, final int resId, long duration) {
        ...
        if ((resId != -1 && (mResId == resId)) || (!TextUtils.isEmpty(resPath) && TextUtils.equals(mResPath, resPath))) {
            return;
        }
     ...
     * @param resPath
     */
    public void myplay(String resPath) {
        //判断是否同一首音乐
        if ((!TextUtils.isEmpty(resPath) && TextUtils.equals(mResPath, resPath))) {
            return;
        }
