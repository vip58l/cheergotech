...
public class okhttp {
    ...
    public static String POST(String path, RequestBody requestBody) {
        ...
        Request request = new Request.Builder()
                .addHeader(Constants.accessToken, TextUtils.isEmpty(Msgconfig.getInstance().getAccessToken()) ? "" : Msgconfig.getInstance().getAccessToken())
     ...
     * put方式提交
     */
    public static String put(String path, RequestBody requestBody) {
        //MediaType JSON = MediaType.parse("application/json;charset=utf-8");
        // RequestBody requestBody = RequestBody.create(JSON, json);
        String cookie = CookieManager.getInstance().getCookie(okhttp.cookie);
        Request request = new Request.Builder()
                .addHeader(Constants.accessToken, TextUtils.isEmpty(Msgconfig.getInstance().getAccessToken()) ? "" : Msgconfig.getInstance().getAccessToken())       //同步accessToken
                .addHeader(okhttp.cookie, TextUtils.isEmpty(cookie) ? "" : cookie)
                .url(path)
                .put(requestBody)
                .build();
     ...
     * del方式提交
     */
    public static String delete(String path, RequestBody requestBody) {
//        MediaType JSON = MediaType.parse("application/json;charset=utf-8");
//        RequestBody requestBody = RequestBody.create(JSON, json);
        String cookie = CookieManager.getInstance().getCookie(okhttp.cookie);
        Request request = new Request.Builder()
                .addHeader(Constants.accessToken, Msgconfig.getInstance().getAccessToken())       //同步accessToken
                .addHeader(okhttp.cookie, TextUtils.isEmpty(cookie) ? "" : cookie)
                .url(path)
                .delete(requestBody)
                .build();
